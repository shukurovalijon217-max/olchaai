...
function selectTtsVoice(profile) {
    if (profile.energy === "energetic" && profile.formality === "casual")
        return "nova";
    if (profile.energy === "calm" && profile.formality === "formal")
        return "onyx";
    if (profile.energy === "moderate" && profile.formality === "formal")
        return "alloy";
...
router.post("/voice/translate", async (req, res) => {
    ...
    const { audioBase64, targetLang = "en", sourceLang } = req.body;
    if (!audioBase64) {
    ...
    try {
        ...
        const detectedLang = transcription.language ?? sourceLang ?? "uz";
        if (!originalText) {
            res.json({
                success: false,
...
router.post("/voice/translate-text", async (req, res) => {
    ...
    const { text, targetLang = "en", voice = "alloy" } = req.body;
    if (!text?.trim()) {
