...
import { challengesTable, challengeParticipantsTable, usersTable, walletsTable, transactionsTable, reelsTable, } from "@workspace/db";
...
async function enrichChallenges(rows, viewerId) {
    if (rows.length === 0)
        return [];
...
router.post("/challenges", requireAuth, async (req, res) => {
    try {
        ...
        const { name, hashtag, category, description, days, prizePool, judgeType, settings } = req.body;
...
router.post("/challenges/:id/join", requireAuth, async (req, res) => {
    try {
        ...
        if (challenge.status !== "active") {
            res.status(400).json({ error: "Challenge tugagan" });
            return;
        }
        if (reelId) {
