...
app.use((req, res, next) => {
    ...
    const isMediaUpload = req.path.includes("/upload") ||
...
app.use((req, res, next) => {
    if (req.method === "GET") {
        ...
        else if (p.startsWith("/api/posts") ||
...
function stripPasswordHash(value, depth = 0) {
    if (depth > 6 || value === null || typeof value !== "object")
        return value;
    if (value instanceof Date)
        return value;
    if (Array.isArray(value))
        return value.map((v) => stripPasswordHash(v, depth + 1));
    const out = {};
...
app.use("/api", (req, res, next) => {
    ...
    res.json = ((body) => originalJson(stripPasswordHash(body)));
    next();
});
app.use("/api", router);
