...
import { reelsTable, reelLikesTable, reelCommentsTable, usersTable, moderationQueueTable, walletsTable, transactionsTable, userInteractionsTable, reelWatchProgressTable, reelCollaboratorsTable, } from "@workspace/db";
...
async function batchEnrichReels(reels, viewerId = 0) {
    if (reels.length === 0)
        return [];
    ...
    const [authors, likedRows, statsMap, views24hRows] = await Promise.all([
        ...
        reelIds.length > 0
            ? db.select({ reelId: userInteractionsTable.contentId, n: count() })
                ...
                .where(and(eq(userInteractionsTable.contentType, "reel"), eq(userInteractionsTable.interactionType, "view"), inArray(userInteractionsTable.contentId, reelIds), gte(userInteractionsTable.createdAt, since24h)))
                .groupBy(userInteractionsTable.contentId)
            : Promise.resolve([]),
    ]);
    const authorMap = new Map(authors.map(a => [a.id, a]));
    const likedSet = new Set(likedRows.map(l => l.reelId));
    const views24hMap = new Map(views24hRows.map(v => [v.reelId, Number(v.n)]));
    ...
    return reels.map(reel => {
        const author = authorMap.get(reel.authorId);
        const authorId = author?.id ?? reel.authorId;
        ...
        const fixUrl = (u) => u && u.startsWith("/") && apiBase ? `${apiBase}${u}` : (u ?? undefined);
        const hlsUrl = fixUrl(reel.hlsUrl);
...
function computeSignalScore(viewsCount, likesCount, views24h) {
...
router.get("/reels/:id/analytics", async (req, res) => {
    try {
        ...
        const [row] = await db.select({ n: count() })
            .from(userInteractionsTable)
            .where(and(eq(userInteractionsTable.contentType, "reel"), eq(userInteractionsTable.interactionType, "view"), eq(userInteractionsTable.contentId, reelId), gte(userInteractionsTable.createdAt, since24h)));
...
router.put("/reels/:id/progress", requireAuth, async (req, res) => {
    try {
        const reelId = Number(req.params.id);
        const userId = req.session.userId;
...
router.get("/reels/continue-watching", requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
...
router.get("/reels/collaborators", requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
...
router.post("/reels/collaborators", requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
        const { inviteeHandle, permission } = req.body;
...
router.delete("/reels/collaborators/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
...
router.post("/reels/:id/view", async (req, res) => {
    try {
        ...
        if (userId)
            void trackQuestAction(userId, "watch_reel");
    }
    catch (err) {
        req.log.error(err);
        res.status(500).json({ error: "Internal server error" });
...
router.get("/reels/:id/versions", requireAuth, async (req, res) => {
    const reelId = Number(req.params.id);
    try {
        const result = await db.execute(sql `SELECT rv.*, u.username, u.display_name AS "displayName", u.avatar_url AS "avatarUrl"
...
router.post("/reels/:id/versions", requireAuth, async (req, res) => {
    const reelId = Number(req.params.id);
    const { note } = req.body;
    try {
        ...
        const result = await db.execute(sql `INSERT INTO reel_versions (reel_id, editor_id, caption, tags, note)
