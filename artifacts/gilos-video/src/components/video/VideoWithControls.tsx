...
export default function VideoWithControls() {
  // Export/recording runtime must always get the bare video (no interactive chrome):
  // it injects window.startRecording before load and/or passes ?export=1.
  const isExportRuntime =
    typeof window !== 'undefined' &&
    (typeof (window as any).startRecording === 'function' ||
      new URLSearchParams(window.location.search).has('export'));
  const isIframed =
    !isExportRuntime && typeof window !== 'undefined' && window.self !== window.top;
