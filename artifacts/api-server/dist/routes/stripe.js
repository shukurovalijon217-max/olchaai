...
import { currencyFromAcceptLanguage, usdCentsToSubunits, isStripeSupported, USD_TO_MAJOR, } from "../lib/currency";
const router = Router();
...
router.get("/stripe/mode", requireAuth, async (req, res) => {
    try {
...
router.post("/stripe/checkout", requireAuth, async (req, res) => {
    try {
        ...
        if (!priceId) {
            res.status(400).json({ error: "priceId talab qilinadi" });
            return;
        }
        ...
        const session = await stripeService.createCheckoutSession(customerId, priceParam, `${baseUrl}/premium?success=true`, `${baseUrl}/premium?canceled=true`);
...
router.post("/stripe/portal", requireAuth, async (req, res) => {
    try {
        ...
        if (!customerId) {
            res.status(400).json({ error: "Stripe mijoz topilmadi" });
            return;
        }
        const baseUrl = process.env.FRONTEND_URL?.replace(/\/$/, "") ||
            (process.env.REPLIT_DOMAINS ? `https://${process.env.REPLIT_DOMAINS.split(",")[0].trim()}` : null) ||
