...
router.post("/spaces", requireAuth, async (req, res) => {
    try {
        ...
        if (!name?.trim()) {
            res.status(400).json({ error: "Nom majburiy" });
            return;
        }
...
router.post("/spaces/:id/join", requireAuth, async (req, res) => {
    try {
...
router.post("/spaces/:id/tasks", requireAuth, async (req, res) => {
    try {
        const spaceId = parseInt(req.params.id);
...
router.patch("/spaces/:id/tasks/:taskId", requireAuth, async (req, res) => {
    try {
...
router.patch("/spaces/:id/canvas", requireAuth, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
