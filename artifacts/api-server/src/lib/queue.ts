...
function buildNativeRedisUrl(): string | null {
...
// ── BullMQ setup (lazy — only when Redis is available) ───────────────────────
type BullQueue = import("bullmq").Queue;
type BullWorker = import("bullmq").Worker;

let notifQueue: BullQueue | null = null;
let analyticsQueue: BullQueue | null = null;
let moderationQueue: BullQueue | null = null;
let bullReady = false;
...
export function enqueueModeration(payload: {
  contentType: "post" | "comment" | "reel" | "story";
  contentId: number;
  text?: string;
  authorId?: number;
}): void {
