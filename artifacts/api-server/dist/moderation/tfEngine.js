/**
 * GILOS SafeGuard — TensorFlow.js Neural Toxicity Engine
 *
...
async function buildModel(tfLib) {
    ...
    // Dropout
    const dropped = tfLib.layers.dropout({ rate: 0.3, name: "drop" }).apply(hidden);
    // Output: 64 → 7 sigmoid
...
export async function initTFEngine() {
    try {
        tf = await loadTF();
...
// ── Inference ─────────────────────────────────────────────────────────────
export async function tfScanContent(text) {
    const inputVec = tokenize(text);
    const hasSignal = inputVec.some(v => v > 0);
    if (!hasSignal)
        return { score: 0, categories: {}, engine: "tensorflow" };
    let preds;
    ...
    const combined = Math.min(Object.values(categories).reduce((a, b) => a + b * 0.2, maxScore * 0.8), 1.0);
    return {
