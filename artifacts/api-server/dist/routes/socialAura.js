...
function scoreToColor(score) {
    if (score >= 90)
        return { color: "#FFD700", label: "Oltin Aura", gradient: "linear-gradient(135deg,#FFD700,#FF8C00)" };
    if (score >= 75)
        return { color: "#C0C0C0", label: "Kumush Aura", gradient: "linear-gradient(135deg,#C0C0C0,#808080)" };
    if (score >= 60)
        return { color: "#9D00FF", label: "Binafsha Aura", gradient: "linear-gradient(135deg,#9D00FF,#4400AA)" };
    if (score >= 45)
        return { color: "#00E5FF", label: "Ko'k Aura", gradient: "linear-gradient(135deg,#00E5FF,#0088CC)" };
    if (score >= 30)
        return { color: "#00FF77", label: "Yashil Aura", gradient: "linear-gradient(135deg,#00FF77,#007744)" };
    if (score >= 15)
        return { color: "#FF6B00", label: "To'q Sariq Aura", gradient: "linear-gradient(135deg,#FF6B00,#FF2D00)" };
...
async function computeAura(userId) {
  ...
  `);
    ...
    const score = Math.min(100, Math.round(posts30 * 4 +
        likesReceived * 2 +
        commentsMade * 1.5 +
        newFollowers * 5 +
        totalReels * 3));
...
router.get("/users/aura", requireAuth, async (req, res) => {
    try {
...
router.get("/users/:id/aura", async (req, res) => {
