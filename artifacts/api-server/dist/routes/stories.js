...
router.get("/stories/:id/reactions", async (req, res) => {
    try {
        ...
        if (story.authorId !== userId) {
            res.status(403).json({ error: "Faqat muallif ko'ra oladi" });
            return;
        }
...
router.post("/stories/:id/reply", async (req, res) => {
    try {
        ...
        if (recipientId === senderId) {
            res.status(400).json({ error: "O'z storyingizga javob bera olmaysiz" });
            return;
        }
        ...
        if (senderConvIds.length > 0) {
            const sharedConvs = await db.select({ conversationId: chatParticipantsTable.conversationId })
                ...
                .where(and(eq(chatParticipantsTable.userId, recipientId), inArray(chatParticipantsTable.conversationId, senderConvIds)));
            ...
            if (sharedIds.length > 0) {
                ...
                if (dmRow)
                    conversationId = dmRow.conversationId;
            }
        }
