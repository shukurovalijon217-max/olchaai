...
import { postsTable, reelsTable, usersTable, postLikesTable, userInteractionsTable, contentAnalysisTable, } from "@workspace/db";
    ...
    try {
        ...
        if (userId) {
            ...
            for (const interaction of interactions) {
                ...
                if (interaction.interactionType === "like")
                    ...
                    w = 1;
                const tags = interaction.contentType === "post"
        ...
        if (viewerId && posts.length > 0) {
            const liked = await db
                ...
                .from(postLikesTable)
                .where(and(inArray(postLikesTable.postId, posts.map(p => p.id)), eq(postLikesTable.userId, viewerId)));
        ...
        const enrichedPosts = posts.map(p => {
            ...
            return {
                ...
                author: enrichUser(author, statsMap.get(p.authorId)),
                tags,
        ...
        const enrichedReels = reels.map(r => {
            ...
            return {
                ...
                author: enrichUser(author, statsMap.get(r.authorId)),
                tags: r.tags ?? [],
                isLiked: false,
            };
        });
        ...
        const suggestedUsers = authors
            ...
            .slice(0, 5)
            .map(u => enrichUser(u, statsMap.get(u.id)));
...
router.get("/ai/analysis/:type/:id", async (req, res) => {
    ...
    try {
        const [analysis] = await db
            ...
            .where(and(eq(contentAnalysisTable.contentType, type), eq(contentAnalysisTable.contentId, Number(id))));
        if (!analysis) {
...
router.post("/ai/analyze-content", async (req, res) => {
    ...
    if (!contentId || !contentType) {
        res.status(400).json({ error: "contentId and contentType required" });
        return;
    }
    try {
        const [existing] = await db
            .select()
            .from(contentAnalysisTable)
            .where(and(eq(contentAnalysisTable.contentType, contentType), eq(contentAnalysisTable.contentId, Number(contentId))));
        if (existing) {
            res.json(existing);
            return;
        }
...
router.get("/ai/suggestions", async (req, res) => {
    try {
        ...
        res.json({
            users: users.map(u => enrichUser(u, statsMap.get(u.id))),
            groups: [],
