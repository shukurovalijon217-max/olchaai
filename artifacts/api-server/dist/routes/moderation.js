...
import { moderationQueueTable, contentReportsTable, postsTable, usersTable } from "@workspace/db";
...
router.post("/moderation/report", requireAuth, async (req, res) => {
    try {
        ...
        if (!contentType || !contentId || !reason) {
            res.status(400).json({ error: "contentType, contentId va reason talab qilinadi" });
            return;
        }
...
router.patch("/admin/moderation/:id/resolve", requireAdmin, async (req, res) => {
    try {
