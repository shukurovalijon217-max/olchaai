...
import { platformExpensesTable, expenseDeductionRequestsTable, walletsTable, } from "@workspace/db/schema";
...
router.post("/admin/platform-expenses", adminOnly, async (req, res) => {
    try {
        const { name, category, amountCents, currency = "USD", period = "monthly", description } = req.body;
        if (!name || !category || !amountCents) {
            res.status(400).json({ error: "name, category, amountCents majburiy" });
            return;
        }
...
router.post("/admin/platform-expenses/deduction-request", adminOnly, async (req, res) => {
    try {
        ...
        if (totalExpenseCents <= 0) {
            res.status(400).json({ error: "Faol xarajatlar yo'q" });
            return;
        }
...
router.post("/admin/platform-expenses/deduction-requests/:id/approve", adminOnly, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const [req_] = await db
            .select()
        ...
        if (adminWallet.earningsBalance < req_.totalExpenseCents) {
            res.status(400).json({ error: "Admin hamyonda yetarli mablag' yo'q" });
            return;
        }
        await db
            .update(walletsTable)
...
router.post("/admin/platform-expenses/deduction-requests/:id/reject", adminOnly, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const [updated] = await db
            .update(expenseDeductionRequestsTable)
            .set({ status: "rejected", notes: req.body.notes ?? null })
            .where(and(eq(expenseDeductionRequestsTable.id, id), eq(expenseDeductionRequestsTable.status, "pending")))
            .returning();
        if (!updated) {
