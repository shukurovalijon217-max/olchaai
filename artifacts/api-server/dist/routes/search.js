            ...
            const [users, posts, reels, products] = await Promise.all([
                (type === "all" || type === "users")
                    ? db.select({
                        ...
                        bio: usersTable.bio,
                    }).from(usersTable).where(and(or(ilike(usersTable.username, like), ilike(usersTable.displayName, like), ilike(usersTable.bio, like)), myId ? ne(usersTable.id, myId) : undefined)).limit(limit)
                    : Promise.resolve([]),
                (type === "all" || type === "posts")
                    ? db.select({
                        id: postsTable.id,
                ...
                (type === "all" || type === "products")
                    ...
                    }).from(productsTable).where(and(or(ilike(productsTable.title, like), ilike(productsTable.description, like)), eq(productsTable.status, "active"))).limit(limit)
                    : Promise.resolve([]),
            ]);
