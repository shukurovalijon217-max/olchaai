...
/* ─── Ban management ────────────────────────────────────────────── */
async function banIp(ip, reason, durationMs, extraStrikes = 1) {
...
async function logSecurityEvent(ip, eventType, path, severity, payload, userAgent, userId) {
    try {
...
function scanValue(value) {
    ...
    for (const [re, type, severity] of pairs) {
        ...
        if (m)
            return { type, severity, sample: m[0].slice(0, 80) };
    }
    return null;
}
function deepScan(obj, depth = 0) {
    ...
    if (Array.isArray(obj)) {
        ...
        for (const key of Object.keys(obj).slice(0, 50)) {
            const r = deepScan(key, depth + 1) ?? deepScan(obj[key], depth + 1);
            if (r)
                return r;
        }
    }
...
async function strike(ip, reason, path, severity, payload, ua, userId) {
...
export async function securityShield(req, res, next) {
    const ip = getIp(req);
    const path = req.path ?? "/";
    ...
    if (queryStr.length > 0) {
        let decoded = "";
        try {
            decoded = decodeURIComponent(queryStr);
        }
        catch {
            decoded = queryStr;
        }
