...
import { searchMusic, jamendoSearch, audiusStream, fetchAudiusHosts } from "../lib/music";
...
async function batchEnrichPosts(posts, viewerId = 0) {
    ...
    return posts.map(post => {
        ...
        const stats = statsMap.get(post.authorId) || { followersCount: 0, followingCount: 0, postsCount: 0, isFollowing: false };
        return {
            ...post,
...
router.get("/music/search", async (req, res) => {
    try {
        ...
        const { results, source } = await cacheAside("music:search", cacheKey, () => searchMusic(q), 300);
        res.setHeader("X-Music-Source", source);
        res.json({ results, source });
    }
    catch (err) {
...
router.get("/music/download/:id", async (req, res) => {
    try {
        const id = String(req.params.id ?? "").replace(/[^a-zA-Z0-9_-]/g, "");
        if (!id) {
            res.status(400).end();
...
router.post("/posts/ai-predict", async (req, res) => {
    try {
        const { mood, mediaType, hasPoll, hotTake } = req.body;
      ...
      `),
            ...
            userId
                ? db.select({ avgLikes: sql `coalesce(avg(${postsTable.likesCount}), 0)::float`, total: sql `count(*)::int` })
        ...
        const overallAvgComments = Number(stats.avg_comments) || 0;
        const overallAvgShares = Number(stats.avg_shares) || 0;
        const segmentAvgs = [];
        if (stats.avg_likes_type != null)
            segmentAvgs.push(Number(stats.avg_likes_type));
        if (mood && stats.avg_likes_mood != null)
            segmentAvgs.push(Number(stats.avg_likes_mood));
        if (hasPoll && stats.avg_likes_poll != null)
            segmentAvgs.push(Number(stats.avg_likes_poll));
        if (hotTake && stats.avg_likes_hot != null)
            segmentAvgs.push(Number(stats.avg_likes_hot));
        const baselineLikes = segmentAvgs.length > 0
            ? segmentAvgs.reduce((a, b) => a + b, 0) / segmentAvgs.length
            : overallAvgLikes;
        const [{ avgLikes: userAvgLikes, total: userPostCount }] = userAvgRes;
        ...
        const [{ count: followerCount }] = followerRes;
...
router.post("/posts/:id/hot-take", async (req, res) => {
    try {
        const postId = Number(req.params.id);
        const { userId, vote } = req.body;
        if (!userId || !vote) {
            res.status(400).json({ error: "userId, vote required" });
            return;
        }
        const { Pool } = await import("pg");
        const pool = new Pool({ connectionString: process.env.DATABASE_URL });
        await pool.query(`INSERT INTO hot_take_votes(post_id,user_id,vote) VALUES($1,$2,$3)
       ON CONFLICT(post_id,user_id) DO UPDATE SET vote=$3`, [postId, userId, vote]);
        const { rows } = await pool.query(`SELECT vote, COUNT(*) as count FROM hot_take_votes WHERE post_id=$1 GROUP BY vote`, [postId]);
        const fire = Number(rows.find((r) => r.vote === "fire")?.count ?? 0);
        const cold = Number(rows.find((r) => r.vote === "cold")?.count ?? 0);
        await pool.end();
...
router.get("/posts/:id/hot-take", async (req, res) => {
    try {
        const postId = Number(req.params.id);
...
router.post("/posts/ai-caption", async (req, res) => {
    try {
        const { mood, mediaType, description } = req.body;
...
router.post("/posts/:id/vote", async (req, res) => {
    try {
        ...
        if (!userId || optionIndex === undefined) {
            res.status(400).json({ error: "userId, optionIndex required" });
            return;
        }
        const { Pool } = await import("pg");
        const pool = new Pool({ connectionString: process.env.DATABASE_URL });
        await pool.query(`INSERT INTO post_votes(post_id,user_id,option_index) VALUES($1,$2,$3)
       ON CONFLICT(post_id,user_id) DO UPDATE SET option_index=$3`, [postId, userId, optionIndex]);
        const { rows } = await pool.query(`SELECT option_index, COUNT(*) as count FROM post_votes WHERE post_id=$1 GROUP BY option_index`, [postId]);
        await pool.end();
        res.json({ votes: rows.map((r) => ({ optionIndex: Number(r.option_index), count: Number(r.count) })), userVote: optionIndex });
    }
    catch (err) {
        req.log.error(err);
        res.status(500).json({ error: "Internal server error" });
...
router.get("/posts/:id/votes", async (req, res) => {
    try {
        ...
        await pool.end();
        res.json({
            votes: voteRows.map((r) => ({ optionIndex: Number(r.option_index), count: Number(r.count) })),
...
router.post("/posts", async (req, res) => {
    try {
        const { content, type, mediaUrl, mediaUrls, overlays, audioName, audioUrl, audioTrimStart, audioTrimEnd, pollQuestion, pollOptions, mood, filterName, tags, midnightOnly, destructAt, geoLat, geoLng, geoRadiusKm, emotionLock, lockedEmotion, liveMoodEnabled, seriesName, seriesOrder, collabCanvasEnabled, collabCanvasId } = req.body;
...
router.get("/posts/series/:name", async (req, res) => {
    try {
...
router.post("/posts/:id/like", async (req, res) => {
    try {
        ...
        if (!isLiked)
            void trackQuestAction(userId, "like_post");
...
router.get("/posts/:id/comments", async (req, res) => {
    try {
        ...
        res.json(comments.map(c => {
            ...
            const stats = statsMap.get(c.authorId) || { followersCount: 0, followingCount: 0, postsCount: 0, isFollowing: false };
            return {
                ...c,
                author: {
...
router.post("/posts/:id/comments", async (req, res) => {
    try {
        ...
        const authorId = req.session?.userId ?? Number(req.body.authorId);
        if (!authorId) {
            res.status(401).json({ error: "Login kerak" });
            return;
        }
