...
export async function generateShareCard(data: ShareCardData): Promise<Blob> {
  ...
  ctx.fillText("@" + data.authorName.toLowerCase().replace(/\s+/g, "_") + " • olcha.uz", 28, 78);
  ...
  ctx.fillText("olcha.uz", W - 28, H - 14);

  return new Promise(res => canvas.toBlob(b => res(b!), "image/png"));
}

export function downloadShareCard(blob: Blob, filename = "olcha-post.png") {
