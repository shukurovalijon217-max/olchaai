...
import { usersTable, followsTable, postsTable, reelsTable } from "@workspace/db";
...
router.get("/users/:id/creator-analytics", async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (isNaN(id)) {
        ...
        for (const row of postViews.rows) {
            const k = String(row.day).slice(0, 10);
            if (dayMap[k])
                dayMap[k].postViews += Number(row.n);
        }
        for (const row of reelViews.rows) {
            const k = String(row.day).slice(0, 10);
            if (dayMap[k])
                dayMap[k].reelViews += Number(row.n);
        }
        for (const row of likesRows.rows) {
            const k = String(row.day).slice(0, 10);
            if (dayMap[k])
                dayMap[k].likes += Number(row.n);
        }
        for (const row of followerGrowth.rows) {
            const k = String(row.day).slice(0, 10);
            if (dayMap[k])
                dayMap[k].newFollowers += Number(row.new_followers);
        }
...
router.get("/users/:userId/posts/:postId/stats", async (req, res) => {
    try {
...
router.get("/users/:userId/reels/:reelId/stats", async (req, res) => {
    try {
        const userId = Number(req.params.userId);
