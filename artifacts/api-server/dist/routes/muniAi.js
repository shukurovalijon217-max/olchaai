...
const router = Router();
const WISDOM_SYSTEM = `Sen "Muni AI" — GILOS platformasining shaxsiy yordamchisisan.
...
const TRADER_SYSTEM = `Sen "Muni AI" — GILOS platformasining bozor va savdo psixologiyasi bo'yicha yordamchisisan.
...
router.post("/muni/chat", async (req, res) => {
    ...
    const { message, mode = "wisdom", history = [] } = req.body;
    if (!message?.trim()) {
