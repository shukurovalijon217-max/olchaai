...
import { usersTable, DEFAULT_FOCUS_SHIELD } from "@workspace/db";
...
function normalize(input) {
    ...
    const allowedUserIds = Array.isArray(input?.allowedUserIds)
        ? input.allowedUserIds.map((n) => Number(n)).filter((n) => Number.isFinite(n) && n > 0)
        : DEFAULT_FOCUS_SHIELD.allowedUserIds;
    return { enabled: !!input?.enabled, startHour, endHour, allowedUserIds };
}
router.get("/focus-shield/my", requireAuth, async (req, res) => {
    try {
...
router.put("/focus-shield", requireAuth, async (req, res) => {
    try {
