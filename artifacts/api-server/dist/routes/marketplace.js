...
import { productsTable, productOrdersTable, productReviewsTable, walletsTable, transactionsTable, usersTable, } from "@workspace/db";
...
router.get("/marketplace/products", async (req, res) => {
    try {
        const { q, category, condition, minPrice, maxPrice, sellerId, sort = "newest" } = req.query;
        const limit = Math.min(Number(req.query.limit ?? 24), 60);
        const offset = Number(req.query.offset ?? 0);
        // Build SQL WHERE conditions — no in-memory filtering
        const conditions = [eq(productsTable.status, "active")];
        if (q)
            conditions.push(or(ilike(productsTable.title, `%${q}%`), ilike(productsTable.description, `%${q}%`)));
        if (category)
            conditions.push(eq(productsTable.category, category));
        if (condition)
            conditions.push(eq(productsTable.condition, condition));
        if (minPrice)
            conditions.push(gte(productsTable.price, Number(minPrice)));
        if (maxPrice)
            conditions.push(lte(productsTable.price, Number(maxPrice)));
        if (sellerId)
...
router.post("/marketplace/products", requireAuth, async (req, res) => {
    try {
        ...
        if (!title?.trim() || !price || price < 1) {
            res.status(400).json({ error: "Nomi va narx talab qilinadi" });
            return;
        }
        ...
        req.log.error(err);
        res.status(500).json({ error: "Mahsulot qo'shishda xato" });
    }
});
// GET /marketplace/products/my
router.get("/marketplace/products/my", requireAuth, async (req, res) => {
    try {
...
router.get("/marketplace/products/:id", async (req, res) => {
    try {
        ...
        req.log.error(err);
        res.status(500).json({ error: "Mahsulotni olishda xato" });
    }
});
...
router.delete("/marketplace/products/:id", requireAuth, async (req, res) => {
    try {
...
router.post("/marketplace/products/:id/buy", requireAuth, async (req, res) => {
    try {
        const productId = Number(req.params.id);
        ...
        if (product.sellerId === req.session.userId) {
            res.status(400).json({ error: "O'z mahsulotingizni sotib ola olmaysiz" });
            return;
        }
        if (product.stock < qty) {
            res.status(400).json({ error: "Yetarli zaxira yo'q" });
            return;
        }
        const totalPrice = product.price * qty;
        const buyerWallet = await getOrCreateWallet(req.session.userId);
        if (buyerWallet.balance < totalPrice) {
            res.status(400).json({ error: "Hamyonda mablag' yetarli emas", required: totalPrice, balance: buyerWallet.balance });
            return;
        }
        ...
        req.log.error(err);
        res.status(500).json({ error: "Sotib olishda xato" });
    }
});
...
router.patch("/marketplace/orders/:id/status", requireAuth, async (req, res) => {
    try {
        ...
        if (order.sellerId !== req.session.userId && order.buyerId !== req.session.userId) {
        ...
        req.log.error(err);
        res.status(500).json({ error: "Holatni yangilashda xato" });
    }
});
...
router.get("/marketplace/products/:id/reviews", async (req, res) => {
    try {
        ...
        req.log.error(err);
        res.status(500).json({ error: "Sharhlarni olishda xato" });
    }
});
...
router.get("/marketplace/seller/:id", async (req, res) => {
    try {
...
router.post("/marketplace/products/:id/reviews", requireAuth, async (req, res) => {
    try {
        const productId = Number(req.params.id);
        const { rating, comment, orderId } = req.body;
        if (!rating || rating < 1 || rating > 5) {
            res.status(400).json({ error: "Reyting 1-5 orasida bo'lishi kerak" });
            return;
        }
        ...
        req.log.error(err);
        res.status(500).json({ error: "Sharh qo'shishda xato" });
    }
});
