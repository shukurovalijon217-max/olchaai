...
import { walletsTable, transactionsTable, paymentMethodsTable, usersTable, } from "@workspace/db";
...
router.post("/wallet/deposit", requireAuth, async (req, res) => {
    try {
        ...
        if (!checkEndpointRateLimit(ip, "wallet_deposit", 10, 60_000)) {
            res.status(429).json({ error: "Juda ko'p urinish. 1 daqiqadan so'ng qayta urining." });
            return;
        }
        ...
        if (!REAL_PAYMENT_METHODS.has(paymentMethod)) {
            res.status(400).json({ error: "Bu to'lov usuli hali ulanmagan (Tez orada)" });
            return;
        }
        const wallet = await getOrCreateWallet(req.session.userId);
        ...
        const baseUrl = process.env.FRONTEND_URL?.replace(/\/$/, "") ||
            ...
            (req.headers.origin && /^https?:\/\/([\w-]+\.)*(olchaai\.com|olchaai\.com|replit\.app|repl\.co)/.test(req.headers.origin) ? req.headers.origin : null) ||
            `https://${req.get("host")}`;
        ...
        const session = await stripeService.createCheckoutSession(customerId, {
            ...
            recurring: null,
        }, `${baseUrl}/wallet?deposit_session={CHECKOUT_SESSION_ID}`, `${baseUrl}/wallet?deposit_canceled=true`, { metadata: { userId: String(req.session.userId), amountTiyin: String(amount), paymentMethod } });
...
router.post("/wallet/transfer", requireAuth, async (req, res) => {
    try {
        ...
        if (toUserId === req.session.userId) {
            res.status(400).json({ error: "O'zingizga pul o'tkaza olmaysiz" });
            return;
        }
        const commissionRate = await getCommissionRate();
        const commission = Math.floor(amount * commissionRate / 100);
...
router.post("/wallet/payment-methods", requireAuth, async (req, res) => {
    try {
...
router.delete("/wallet/payment-methods/:id", requireAuth, async (req, res) => {
    try {
...
router.post("/wallet/tip", requireAuth, async (req, res) => {
    try {
        const data = tipSchema.parse(req.body);
        if (data.toUserId === req.session.userId) {
            res.status(400).json({ error: "O'zingizga tip yubora olmaysiz" });
            return;
        }
        const fromWallet = await getOrCreateWallet(req.session.userId);
        if (fromWallet.balance < data.amount) {
            res.status(400).json({ error: "Hamyonda yetarli mablag' yo'q" });
            return;
        }
...
router.post("/admin/wallet/bonus", requireAuth, async (req, res) => {
    try {
