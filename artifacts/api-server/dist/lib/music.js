 ...
 */
export async function fetchAudiusHosts() {
    try {
        const r = await fetch("https://api.audius.co", { signal: AbortSignal.timeout(8000) });
        if (!r.ok)
            return AUDIUS_HOSTS;
        const d = await r.json();
        const hosts = (d.data ?? []).filter((h) => typeof h === "string" && h.startsWith("https://"));
...
export async function audiusSearch(q, limit = 40, hosts) {
    const searchHosts = hosts ?? AUDIUS_HOSTS;
    for (const host of searchHosts) {
        try {
            const r = await fetch(`${host}/v1/tracks/search?query=${encodeURIComponent(q)}&limit=${limit}&app_name=${AUDIUS_APP}`, { signal: AbortSignal.timeout(10000), redirect: "follow" });
            if (!r.ok)
                continue;
            const d = await r.json();
            if ((d.data ?? []).length > 0)
...
export async function audiusStream(id) {
    for (const host of AUDIUS_HOSTS) {
        try {
            const r = await fetch(`${host}/v1/tracks/${id}/stream?app_name=${AUDIUS_APP}`, { signal: AbortSignal.timeout(20000), redirect: "follow" });
            if (!r.ok || !r.body)
                continue;
...
export async function jamendoSearch(q, limit = 40) {
    try {
...
export async function searchMusic(q, limit = 40, hosts) {
    // 1. Try Audius
