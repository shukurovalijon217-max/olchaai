...
const requestTimestamps = []; // rolling window for global RPS
let peakRps = 0;
let totalRequests = 0;
...
/* ─── Main middleware ─────────────────────────────────────────────── */
export function aiAutoScaleMiddleware(req, res, next) {
    totalRequests++;
    activeConnections++;
...
export function getScaleMetrics() {
    ...
    const memPressure = memMB > MEMORY_CRITICAL_MB ? "critical"
        : memMB > MEMORY_WARN_MB ? "warn" : "ok";
    const rpsPressure = rps > GLOBAL_CRITICAL_RPS ? "critical"
        : rps > GLOBAL_WARNING_RPS ? "warn" : "ok";
    const health = memPressure === "critical" || rpsPressure === "critical" ? "overloaded"
