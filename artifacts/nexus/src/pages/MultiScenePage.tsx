    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-white/40 text-xs">Qadam {path.length}/{Math.max(path.length, branches.length)}</div>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="relative flex-1 bg-black flex items-center justify-center min-h-[300px]">
          {current?.videoUrl ? (
            ...
            <div className="flex flex-col items-center gap-3 text-center p-8">
              ...
              <p className="text-white/40 text-sm">{current?.choiceText ?? "Boshlang'ich nuqta"}</p>
              {path.length > 1 && (
        ...
        <div className="p-4 space-y-3">
          {choices.length > 0 ? (
            <>
              <p className="text-white/60 text-xs text-center">Qaysi yo'lni tanlaysiz?</p>
            ...
            <div className="text-center py-4">
              ...
              <p className="text-white/60 text-sm">Siz oxirga yetdingiz!</p>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }}
        ...
        <h3 className="text-white font-bold text-lg mb-4">Yangi Stsenariy</h3>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-white font-bold text-xl flex items-center gap-2">
                  <GitBranch className="w-5 h-5 text-violet-400" /> Multi-Ssenariy
                </h1>
                <p className="text-white/50 text-xs mt-0.5">Siz boshqaradigan interaktiv video</p>
              </div>
              {user && (
            ...
            <div className="flex gap-2">
              {(["explore", "mine"] as const).map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${tab === t ? "bg-violet-500 text-white" : "bg-white/5 text-white/60 hover:bg-white/10"}`}>
                  {t === "explore" ? <><Globe className="w-3 h-3 inline mr-1" />Kashf et</> : <><Lock className="w-3 h-3 inline mr-1" />Mening</>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="text-white/40 text-sm">Hali stsenariylar yo'q</p>
