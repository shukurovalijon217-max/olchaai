...
// Cache credentials to avoid hammering Metered on every call setup
// RTCIceServer is a browser type — use plain object in Node
type IceServer = { urls: string; username?: string; credential?: string };

let cachedIce: IceServer[] | null = null;
...
async function fetchMeteredIce(): Promise<IceServer[]> {
  ...
  try {
    ...
    console.warn("[webrtc] Metered TURN fetch failed, using fallback:", (err as Error).message);
