/**
 * GILOS Media Fingerprinter — Node.js wrapper for the C++ pHash engine.
 *
 * Usage:
...
const BINARY = path.resolve(process.cwd().endsWith(path.join("artifacts", "api-server"))
    ? path.resolve(process.cwd(), "../..")
    : process.cwd(), "artifacts/api-server/src/cpp/mediaHasher");
...
export function hashImagePixels(pixels) {
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on("close", (code) => {
            if (code !== 0)
                return reject(new Error(`mediaHasher exited ${code}: ${stderr}`));
            try {
                resolve(JSON.parse(stdout.trim()));
            }
            catch (e) {
                reject(new Error(`Invalid JSON from mediaHasher: ${stdout}`));
            }
        });
...
export function hashSimilarity(h1, h2) {
