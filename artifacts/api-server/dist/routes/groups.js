...
import { groupsTable, groupMembersTable, groupPostsTable, groupPostLikesTable, groupPostReactionsTable, groupPostCommentsTable, groupPostCommentLikesTable, groupPollsTable, groupPollVotesTable, groupPostBookmarksTable, groupPostReportsTable, usersTable, } from "@workspace/db";
...
router.post("/groups", async (req, res) => {
    try {
        const { name, description, coverUrl, avatarUrl, isPrivate, category, privacyLevel, joinType, groupType, icon, themeColor, maxMembers, settings, } = req.body;
...
router.put("/groups/:id", async (req, res) => {
    try {
        ...
        const { name, description, coverUrl, category, privacyLevel, joinType, icon, themeColor, maxMembers, settings, } = req.body;
        const updateData = {};
        if (name?.trim())
        ...
        if (maxMembers !== undefined)
            updateData.maxMembers = Number(maxMembers) || 0;
...
router.patch("/groups/:id/members/:userId/mute", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
        const targetUserId = Number(req.params.userId);
        const requesterId = req.session?.userId;
...
router.get("/groups/:id/posts", async (req, res) => {
    try {
        ...
        let query = db
            ...
            .$dynamic();
        let posts = await query.orderBy(sortBy === "popular"
            ? desc(groupPostsTable.reactionsCount)
            : desc(groupPostsTable.createdAt)).limit(limit);
        if (filterType === "media")
            posts = posts.filter(p => !!p.mediaUrl);
        if (filterType === "polls")
            posts = posts.filter(p => p.postType === "poll");
...
/* ── Delete group post ──────────────────────────────────────── */
router.delete("/groups/:id/posts/:postId", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
...
router.post("/groups/:id/posts/:postId/pin", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
...
router.post("/groups/:id/posts/:postId/like", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        if (isNaN(postId)) {
            res.status(400).json({ error: "Invalid id" });
...
router.post("/groups/:id/posts/:postId/react", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
        if (!userId) {
        ...
        const reactionCounts = await db
            .select({ reactionType: groupPostReactionsTable.reactionType, count: sql `count(*)` })
...
router.get("/groups/:id/posts/:postId/reactions", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const reactionCounts = await db
            .select({ reactionType: groupPostReactionsTable.reactionType, count: sql `count(*)` })
...
/* ── Get post comments ──────────────────────────────────────── */
router.get("/groups/:id/posts/:postId/comments", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
        const limit = Math.min(Number(req.query.limit) || 50, 100);
...
/* ── Post a comment ─────────────────────────────────────────── */
router.post("/groups/:id/posts/:postId/comments", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
        if (!userId) {
...
/* ── Delete a comment ───────────────────────────────────────── */
router.delete("/groups/:id/posts/:postId/comments/:commentId", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
...
router.post("/groups/:id/posts/:postId/comments/:commentId/like", async (req, res) => {
    try {
        const commentId = Number(req.params.commentId);
        const userId = req.session?.userId;
        if (!userId) {
...
/* ── Get group polls ────────────────────────────────────────── */
router.get("/groups/:id/polls", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
        const userId = req.session?.userId;
...
/* ── Create group poll ──────────────────────────────────────── */
router.post("/groups/:id/polls", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
        const userId = req.session?.userId;
        if (!userId) {
...
/* ── Vote on a poll ─────────────────────────────────────────── */
router.post("/groups/:id/polls/:pollId/vote", async (req, res) => {
    try {
...
router.post("/groups/:id/posts/:postId/bookmark", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
        if (!userId) {
...
router.get("/groups/:id/bookmarks", async (req, res) => {
    try {
        const groupId = Number(req.params.id);
        const userId = req.session?.userId;
        if (!userId) {
        ...
        const bookmarks = await db
            ...
            .innerJoin(usersTable, eq(groupPostsTable.authorId, usersTable.id))
            .where(and(eq(groupPostBookmarksTable.userId, userId), eq(groupPostsTable.groupId, groupId)))
...
/* ── Report a post ──────────────────────────────────────────── */
router.post("/groups/:id/posts/:postId/report", async (req, res) => {
    try {
        const postId = Number(req.params.postId);
        const userId = req.session?.userId;
        if (!userId) {
...
/* ── Get invite link ────────────────────────────────────────── */
router.get("/groups/:id/invite-link", async (req, res) => {
    try {
        const id = Number(req.params.id);
        const userId = req.session?.userId;
        if (!userId) {
...
router.post("/groups/:id/invite-link/regenerate", async (req, res) => {
    try {
        const id = Number(req.params.id);
        const userId = req.session?.userId;
        if (!userId) {
