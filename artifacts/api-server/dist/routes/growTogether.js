...
router.get("/grow-together/goal", requireAuth, async (req, res) => {
    try {
        const result = await db.execute(sql `SELECT * FROM grow_together_goals WHERE user_id = ${req.session.userId} ORDER BY created_at DESC LIMIT 1`);
...
router.post("/grow-together/goal", requireAuth, async (req, res) => {
    const { goalText, category = "general" } = req.body;
    if (!goalText?.trim()) {
        res.status(400).json({ error: "Maqsad matni kerak" });
        return;
    }
    try {
        await db.execute(sql `DELETE FROM grow_together_goals WHERE user_id = ${req.session.userId}`);
        const result = await db.execute(sql `INSERT INTO grow_together_goals (user_id, goal_text, category)
...
router.get("/grow-together/matches", requireAuth, async (req, res) => {
    try {
        const myGoal = await db.execute(sql `SELECT * FROM grow_together_goals WHERE user_id = ${req.session.userId} LIMIT 1`);
...
router.post("/grow-together/connect/:partnerId", requireAuth, async (req, res) => {
    const partnerId = Number(req.params.partnerId);
    if (!partnerId) {
        res.status(400).json({ error: "Partner ID kerak" });
        return;
    }
    try {
        const myGoal = await db.execute(sql `SELECT goal_text FROM grow_together_goals WHERE user_id = ${req.session.userId} LIMIT 1`);
...
router.get("/grow-together/connections", requireAuth, async (req, res) => {
    try {
        const result = await db.execute(sql `
      SELECT c.*,
...
router.delete("/grow-together/connect/:partnerId", requireAuth, async (req, res) => {
    const partnerId = Number(req.params.partnerId);
    try {
