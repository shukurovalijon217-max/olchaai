...
export function generateUploadSession(baseUrl) {
...
async function uploadBuffer(uuid, buffer, contentType) {
    configure();
    return new Promise((resolve, reject) => {
...
async function saveToDb(uuid, cloudinaryUrl, resourceType) {
    try {
        await db.execute(sql `INSERT INTO upload_sessions (uuid, cloudinary_url, created_at)
...
export async function uploadBufferToCloudinary(uuid, buffer, contentType) {
...
export async function streamToCloudinary(uuid, readableStream, contentType) {
    ...
    const cloudinaryUrl = await new Promise((resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream({ public_id: uuid, resource_type: resourceType, overwrite: true }, (error, result) => {
            if (error)
                return reject(error);
            resolve(result.secure_url);
        });
 ...
 */
export async function getCloudinaryUrl(uuid) {
    try {
        const rows = await db.execute(sql `SELECT cloudinary_url FROM upload_sessions WHERE uuid = ${uuid}`);
        const r = rows.rows?.[0];
        if (r?.cloudinary_url)
            return r.cloudinary_url;
    }
    catch (err) {
