...
const getResend = () => {
    ...
    if (!key)
        throw new Error("RESEND_API_KEY muhit o'zgaruvchisi o'rnatilmagan");
...
router.post("/auth/send-otp", async (req, res) => {
    try {
        ...
        if (!email || !email.includes("@")) {
            res.status(400).json({ error: "Email manzil noto'g'ri" });
            return;
        }
      ...
      `;
        ...
        if (sendError) {
            ...
            res.status(500).json({ error: "Email yuborishda xato. Keyinroq urinib ko'ring." });
            return;
        }
...
router.post("/auth/verify-otp", async (req, res) => {
    try {
        ...
        if (!email || !otp) {
            res.status(400).json({ error: "Email va kod kiritilishi shart" });
            return;
        }
        const now = new Date();
        const [record] = await db.select().from(emailVerifications)
            .where(and(eq(emailVerifications.email, email), eq(emailVerifications.otp, otp), eq(emailVerifications.verified, false), gt(emailVerifications.expiresAt, now)));
        if (!record) {
...
router.post("/auth/register", async (req, res) => {
    try {
        ...
        if (normalizedPhone.length < 9) {
            res.status(400).json({ error: "Telefon raqami noto'g'ri" });
            return;
        }
        ...
        if (existingPhone.length > 0) {
            res.status(409).json({ error: "Bu telefon raqami allaqachon ro'yxatdan o'tgan. Har bir raqamga faqat 1 ta akkount." });
            return;
        }
...
router.post("/auth/login", async (req, res) => {
    try {
        ...
        if (!identifier || !password) {
            res.status(400).json({ error: "Email/username va parol kiritilishi shart" });
            return;
        }
        const ip = req.headers["x-forwarded-for"]?.split(",")[0]?.trim() ?? req.socket.remoteAddress ?? "unknown";
        // Brute-force check
        ...
        if (!user) {
            ...
            res.status(401).json({ error: "Email/username yoki parol noto'g'ri" });
            return;
        }
        if (!user.passwordHash) {
            res.status(401).json({ error: "Eski akkaunt — admin bilan bog'laning" });
            return;
        }
...
router.patch("/auth/profile", async (req, res) => {
    try {
        ...
        const { displayName, bio, avatarUrl, coverUrl, country, timezone } = req.body;
        const updates = {};
        ...
        if (timezone !== undefined)
            updates.timezone = timezone.trim() || null;
