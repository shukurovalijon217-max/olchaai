...
router.get("/admin/ai-actions", requireAdmin, async (req, res) => {
    ...
    `);
        const exRow = (r) => r?.rows?.[0] ?? (Array.isArray(r) ? r[0] : {});
        const exRows = (r) => r?.rows ?? (Array.isArray(r) ? r : []);
        res.json({
            actions,
...
router.post("/admin/ai-actions/run", requireAdmin, async (req, res) => {
    try {
      ...
      LIMIT 20
    `);
        const flagged = Array.isArray(flaggedPosts) ? flaggedPosts : flaggedPosts?.rows ?? [];
        ...
        if (flagged.length > 0)
            results.push(`${flagged.length} ta flaglangan post olib tashlandi`);
    ...
    `);
      ...
      `);
        ...
        if (warned.length > 0)
            results.push(`${warned.length} ta foydalanuvchi bloklandi`);
    ...
    `);
      ...
      `);
            ...
            if (staleCount > 0)
                results.push(`${staleCount} ta eskirgan e'lon arxivlandi`);
        }
...
router.get("/admin/ai-actions/stats", requireAdmin, async (req, res) => {
    ...
    `);
        ...
        res.json({
            ...
            byType: stats,
            autoban: stats.filter((s) => s.action_type === "auto_ban").reduce((a, s) => a + Number(s.count), 0),
            removedPosts: stats.filter((s) => s.action_type === "remove_post").reduce((a, s) => a + Number(s.count), 0),
            overridden: 0,
        });
    }
    catch (err) {
