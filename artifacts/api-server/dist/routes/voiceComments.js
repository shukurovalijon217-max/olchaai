...
async function enrichVoiceComment(vc) {
...
router.post("/voice-comments/upload-url", requireAuth, async (req, res) => {
    try {
        if (!isR2Enabled()) {
...
router.post("/posts/:id/voice-comments", requireAuth, async (req, res) => {
    try {
        const postId = Number(req.params.id);
        if (isNaN(postId)) {
            res.status(400).json({ error: "Noto'g'ri post ID" });
...
router.delete("/posts/:postId/voice-comments/:id", requireAuth, async (req, res) => {
    try {
        ...
        if (vc.authorId !== req.session.userId) {
