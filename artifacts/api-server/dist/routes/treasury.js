...
router.get("/admin/treasury", requireAdmin, async (req, res) => {
    ...
    `);
        const todayRow = todayRes.rows?.[0] ?? (Array.isArray(todayRes) ? todayRes[0] : {});
        const weekRow = weekRes.rows?.[0] ?? (Array.isArray(weekRes) ? weekRes[0] : {});
        const monthRow = monthRes.rows?.[0] ?? (Array.isArray(monthRes) ? monthRes[0] : {});
        res.json({
            treasury: treasury ?? {
...
router.post("/admin/treasury/withdraw", requireAdmin, async (req, res) => {
    try {
        const { amount, method, details, description } = req.body;
        if (!amount || amount <= 0) {
