...
router.get("/admin/dashboard", async (req, res) => {
    try {
        ...
        const regions = await db.select({ region: usersTable.country, count: sql `count(*)::int` })
...
router.get("/admin/analytics", async (req, res) => {
    try {
        ...
        const toMap = (r) => new Map(rows(r).map((row) => [row.date, row.count]));
        ...
        const [commentStats] = await db.select({ totalComments: sql `count(*)::int` })
        ...
        const authorIds = [...new Set(topPosts.map(p => p.authorId).filter(Boolean))];
        ...
        const enrichedTop = topPosts.map((p) => {
            ...
            const stats = statsMap.get(p.authorId) || { followersCount: 0, followingCount: 0, postsCount: 0, isFollowing: false };
...
router.get("/admin/ai-usage", async (req, res) => {
    try {
        ...
        const [freeAtLimit] = await db.select({ count: sql `count(*)::int` }).from(usersTable)
...
router.patch("/admin/wallet/withdrawals/:id", async (req, res) => {
    try {
        const id = Number(req.params.id);
        ...
        if (tx.status !== "pending") {
            res.status(400).json({ error: "So'rov allaqachon ko'rib chiqilgan" });
            return;
        }
        if (action === "approve") {
...
router.patch("/admin/commission", async (req, res) => {
    try {
        ...
        if (typeof rate !== "number" || rate < 0 || rate > 100) {
            res.status(400).json({ error: "rate 0–100 orasida bo'lishi kerak" });
            return;
        }
...
router.get("/admin/commission/stats", async (req, res) => {
    try {
        ...
        const [total] = await db
            .select({ total: sql `coalesce(sum(amount),0)::int`, count: sql `count(*)::int` })
            .from(tt)
        ...
        const [monthly] = await db
            .select({ total: sql `coalesce(sum(amount),0)::int` })
            .from(tt)
...
router.put("/admin/premium-config", async (req, res) => {
    try {
        const { monthlyPriceCents, yearlyDiscountPercent } = req.body;
        if (monthlyPriceCents !== undefined && (typeof monthlyPriceCents !== "number" || monthlyPriceCents < 100 || monthlyPriceCents > 99900)) {
            res.status(400).json({ error: "monthlyPriceCents 100–99900 orasida bo'lishi kerak" });
            return;
        }
        ...
        if (!productId) {
            res.status(400).json({ error: "Stripe mahsulot topilmadi. Avval /api/admin/stripe/seed ni ishga tushiring" });
            return;
        }
...
router.get("/admin/audit-logs", async (req, res) => {
    ...
    `;
        ...
        const rows = (r) => r?.rows ?? [];
        const items = rows(queryRows).map((row) => ({
            id: row.id,
            userId: row.user_id,
