...
router.get("/anon/zones", requireAuth, async (req, res) => {
    try {
...
router.get("/anon/zones/:id/posts", requireAuth, async (req, res) => {
    try {
...
router.post("/anon/zones/:id/posts", requireAuth, async (req, res) => {
    try {
        const zoneId = Number(req.params.id);
        if (isNaN(zoneId)) {
            res.status(400).json({ error: "Noto'g'ri ID" });
...
router.post("/anon/zones/:id/posts/:postId/like", requireAuth, async (req, res) => {
    try {
