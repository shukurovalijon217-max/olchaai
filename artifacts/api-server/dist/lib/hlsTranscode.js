...
export function parseGcsPath(path) {
    if (!path.startsWith("/"))
        path = `/${path}`;
    const parts = path.split("/");
    return { bucketName: parts[1], objectName: parts.slice(2).join("/") };
}
async function signGetUrl(bucketName, objectName) {
...
async function resolveDownloadUrl(videoUrl) {
...
export async function transcodeReelToHLS(reelId, videoUrl) {
