...
function rulesScan(text) {
    ...
    const combinedScore = Math.min(Object.values(categories).reduce((a, b) => a + b * 0.3, maxScore * 0.7), 1.0);
...
async function openaiModerationScan(text) {
    try {
        ...
        for (const [key, score] of Object.entries(result.category_scores)) {
            ...
            if (s > existing)
                categories[mapped] = Math.round(s * 100) / 100;
            if (s > maxScore) {
                maxScore = s;
                topCategory = mapped;
            }
        ...
        const verdict = result.flagged ? (finalScore >= 0.7 ? "violation" : "suspicious")
...
export async function scanContentAsync(text) {
    if (!text || text.trim().length === 0) {
    ...
    if (openaiResult) {
        const mergedCats = { ...openaiResult.categories };
        ...
        const verdict = finalScore >= 0.65 ? "violation" : finalScore >= 0.3 ? "suspicious" : "clean";
