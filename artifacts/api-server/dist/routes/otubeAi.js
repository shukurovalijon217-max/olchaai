...
async function gateAI(userId, res) {
...
function contextFrom(body) {
    const { caption, category, tags, transcript } = body;
    const parts = [
...
router.post("/otube/ai/title", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson("You generate catchy short-video titles. Return ONLY valid JSON: {\"titles\":[\"...\",\"...\",\"...\"]} with 5 options, in Uzbek, punchy, under 60 chars each.", ctx);
...
router.post("/otube/ai/description", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson("You write engaging short-video descriptions. Return ONLY valid JSON: {\"description\":\"...\"} in Uzbek, 2-3 sentences, under 300 chars.", ctx);
        const { description } = parseJson(raw, {});
        await incrementAIUsage(req.session.userId);
...
router.post("/otube/ai/tags", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson("Suggest content tags for search/discovery. Return ONLY valid JSON: {\"tags\":[\"...\"]} with 6-10 lowercase single/double-word tags, in Uzbek or universal terms (no # symbol).", ctx);
...
router.post("/otube/ai/hashtags", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson('Suggest trending-style hashtags with an estimated reach tier. Return ONLY valid JSON: {"hashtags":[{"tag":"#example","reach":"120K+"}]} with 6 items, hashtags in Uzbek/universal terms, reach as a short string like "50K+" or "1.2M+".', ctx);
        const { hashtags } = parseJson(raw, {});
        await incrementAIUsage(req.session.userId);
...
router.post("/otube/ai/hooks", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson("Write attention-grabbing opening lines (hooks) for the first 3 seconds of a short video script. Return ONLY valid JSON: {\"hooks\":[\"...\"]} with 4 short Uzbek hooks.", ctx);
        const { hooks } = parseJson(raw, {});
        await incrementAIUsage(req.session.userId);
...
router.post("/otube/ai/director-tips", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson("Give short-video editing/directing tips (pacing, cuts, framing, sound) tailored to the content. Return ONLY valid JSON: {\"tips\":[\"...\"]} with 4 concise Uzbek tips.", ctx);
        const { tips } = parseJson(raw, {});
        await incrementAIUsage(req.session.userId);
...
router.post("/otube/ai/best-time", requireAuth, async (req, res) => {
    try {
...
router.post("/otube/ai/color-correction", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
        const ctx = contextFrom(req.body);
        const raw = await askJson('Suggest a color grade for this short video. Return ONLY valid JSON: {"filters":{"brightness":1.0,"contrast":1.0,"saturation":1.0,"temperature":0},"note":"1 short sentence in Uzbek explaining the look"}. brightness/contrast/saturation are multipliers around 1.0 (range 0.8-1.3), temperature is -20 to 20 (warm positive, cool negative).', ctx, 200);
...
router.post("/otube/ai/voice-caption", requireAuth, async (req, res) => {
    try {
        if (!(await gateAI(req.session.userId, res)))
            return;
...
router.post("/otube/ai/dub", requireAuth, async (req, res) => {
    try {
        if (!await gateAI(req.session.userId, res))
            return;
        const { caption, targetLang = "uz" } = req.body;
        if (!caption?.trim()) {
            res.status(400).json({ error: "Caption kerak" });
            return;
        }
        const LANG_NAMES = {
        ...
        const translated = await askJson(`You are a professional translator. Translate the given text to ${langName}. Return ONLY the translated text, no quotes, no explanation.`, caption.trim(), 400);
