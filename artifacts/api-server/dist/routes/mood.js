...
router.post("/mood", requireAuth, async (req, res) => {
    try {
        ...
        if (!mood) {
            res.status(400).json({ error: "Kayfiyat majburiy" });
            return;
        }
...
router.get("/mood/following/energy", requireAuth, async (req, res) => {
    try {
        ...
        const rows = await db
            ...
            .where(and(inArray(userMoodsTable.userId, followeeIds), eq(userMoodsTable.isPublic, true), gt(userMoodsTable.expiresAt, new Date())))
...
router.get("/mood/my", requireAuth, async (req, res) => {
    try {
