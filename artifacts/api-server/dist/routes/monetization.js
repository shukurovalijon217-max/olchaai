...
import { monetizationConfigTable, contentEarningsTable, payoutRequestsTable, walletsTable, transactionsTable, usersTable, creatorMonetizationTable, followsTable, } from "@workspace/db";
...
async function getConfig() {
    if (_cfgCache && Date.now() - _cfgCacheAt < 60_000)
        return _cfgCache;
...
function burstUserMonCache(userId) { _userMonCache.delete(userId); }
const TYPE_MULT = {
...
export async function accumulateViewEarning(contentType, contentId, authorId) {
    try {
        ...
        const multiplier = cfg[multKey] / 10;
        ...
        const [existing] = await db
            ...
            .where(and(eq(contentEarningsTable.contentType, contentType), eq(contentEarningsTable.contentId, contentId)))
            .limit(1);
        if (!existing) {
...
router.put("/admin/monetization/config", requireAdmin, async (req, res) => {
    try {
        ...
        const { enabled, revenuePerMille, creatorSharePercent, minViewsThreshold, videoRateMultiplier, reelRateMultiplier, musicRateMultiplier, movieRateMultiplier, minPayoutAmount, minFollowers, minTotalViews, minContentCount, autoApprove, } = req.body;
        const patch = { updatedAt: new Date(), updatedBy: adminId };
        if (enabled !== undefined)
            patch.enabled = Boolean(enabled);
        if (revenuePerMille !== undefined)
            patch.revenuePerMille = Math.max(0, Number(revenuePerMille));
        if (creatorSharePercent !== undefined)
            patch.creatorSharePercent = Math.max(0, Math.min(100, Number(creatorSharePercent)));
        if (minViewsThreshold !== undefined)
            patch.minViewsThreshold = Math.max(0, Number(minViewsThreshold));
        if (videoRateMultiplier !== undefined)
            patch.videoRateMultiplier = Math.max(1, Number(videoRateMultiplier));
        if (reelRateMultiplier !== undefined)
            patch.reelRateMultiplier = Math.max(1, Number(reelRateMultiplier));
        if (musicRateMultiplier !== undefined)
            patch.musicRateMultiplier = Math.max(1, Number(musicRateMultiplier));
        if (movieRateMultiplier !== undefined)
            patch.movieRateMultiplier = Math.max(1, Number(movieRateMultiplier));
        if (minPayoutAmount !== undefined)
            patch.minPayoutAmount = Math.max(0, Number(minPayoutAmount));
        if (minFollowers !== undefined)
            patch.minFollowers = Math.max(0, Number(minFollowers));
        if (minTotalViews !== undefined)
            patch.minTotalViews = Math.max(0, Number(minTotalViews));
        if (minContentCount !== undefined)
            patch.minContentCount = Math.max(0, Number(minContentCount));
        if (autoApprove !== undefined)
            patch.autoApprove = Boolean(autoApprove);
...
router.patch("/admin/monetization/payouts/:id", requireAdmin, async (req, res) => {
    try {
        ...
        if (action === "approve") {
            ...
            if (wallet.earningsBalance < payout.amount) {
...
router.patch("/admin/monetization/applications/:id", requireAdmin, async (req, res) => {
    try {
        ...
        if (app.status !== "applied") {
            res.status(400).json({ error: "Not in applied state" });
            return;
        }
...
router.post("/creator/monetization/apply", requireAuth, async (req, res) => {
    try {
        ...
        if (followers < (cfg?.minFollowers ?? 1000)) {
            res.status(400).json({ error: `Kamida ${cfg?.minFollowers ?? 1000} ta obunachi kerak` });
            return;
        }
        if (totalViews < (cfg?.minTotalViews ?? 10000)) {
            res.status(400).json({ error: `Kamida ${(cfg?.minTotalViews ?? 10000).toLocaleString()} ta ko'rish kerak` });
            return;
        }
        ...
        if (existing?.status === "active") {
            res.status(400).json({ error: "Monetizatsiya allaqachon yoqilgan" });
            return;
        }
...
router.post("/creator/monetization/payout", requireAuth, async (req, res) => {
    try {
        ...
        if (status !== "active") {
            res.status(403).json({ error: "Monetizatsiya dasturida faol bo'lmagan kreatorlar pul so'ray olmaydi" });
            return;
        }
