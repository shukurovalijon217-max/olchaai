...
router.post("/anon-inbox/:userId/ask", async (req, res) => {
    try {
        ...
        if (!checkAskRateLimit(ip)) {
            res.status(429).json({ error: "Juda ko'p so'rov. Birozdan so'ng qayta urinib ko'ring." });
            return;
        }
        ...
        if (scan.autoBlock) {
            res.status(422).json({ error: "Savol avtomatik bloklandi — qoidalarga zid kontent." });
            return;
        }
...
// Own inbox
router.get("/anon-inbox", async (req, res) => {
...
router.post("/anon-inbox/:id/answer", async (req, res) => {
    const userId = requireAuth(req, res);
    if (!userId)
        return;
    try {
