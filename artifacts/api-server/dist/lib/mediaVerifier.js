...
const BATCH = 50;
export async function verifyMediaUrls() {
    try {
        ...
        const nonR2Posts = await db
            ...
            .from(postsTable)
            .where(and(isNotNull(postsTable.mediaUrl), eq(postsTable.mediaVerified, true), inArray(postsTable.type, [...MEDIA_TYPES])))
            .limit(500);
        ...
        const r2Posts = await db
            ...
            .where(and(isNotNull(postsTable.mediaUrl), eq(postsTable.mediaVerified, true), inArray(postsTable.type, [...MEDIA_TYPES])))
            .limit(100);
        const r2Candidates = r2Posts.filter(p => p.mediaUrl && p.mediaUrl.startsWith(R2_DOMAIN));
        const broken = [];
        await Promise.allSettled(r2Candidates.map(async (p) => {
            try {
