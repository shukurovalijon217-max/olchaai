...
class SystemMonitorImpl {
    ...
    record(endpoint, statusCode, latencyMs) {
        const r = this.get(endpoint);
        const now = Date.now();
        r.total++;
        r.latencies.push(latencyMs);
        if (r.latencies.length > 200)
            r.latencies.shift();
        ...
        if (statusCode >= 500) {
            ...
            if (r.circuit === "closed" && r.recentErrorTimes.length >= CIRCUIT_OPEN_ERRORS) {
                r.circuit = "open";
                r.circuitOpenAt = now;
                this.emit(endpoint, "circuit_opened", `${r.recentErrorTimes.length} errors/${Math.round(CIRCUIT_WINDOW_MS / 1000)}s`);
            }
        }
    ...
    health() {
        const now = Date.now();
        const endpoints = Array.from(this.map.entries()).map(([ep, r]) => {
    ...
    traffic() {
        ...
        while (this.recentTimes.length && this.recentTimes[0] < cutoff)
...
export function normalisePath(method, path) {
