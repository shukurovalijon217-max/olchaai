...
router.post("/coview/rooms", requireAuth, async (req, res) => {
    try {
        const { contentType, contentId } = req.body;
        if (!contentType || !contentId) {
            res.status(400).json({ error: "contentType va contentId talab qilinadi" });
            return;
        }
...
router.get("/coview/rooms/:code", requireAuth, async (req, res) => {
    try {
...
router.post("/coview/rooms/:code/join", requireAuth, async (req, res) => {
    try {
        const [room] = await db.select().from(coViewRoomsTable)
            .where(eq(coViewRoomsTable.inviteCode, req.params.code.toUpperCase()));
        if (!room) {
...
router.delete("/coview/rooms/:code/leave", requireAuth, async (req, res) => {
    try {
        const [room] = await db.select().from(coViewRoomsTable)
            .where(eq(coViewRoomsTable.inviteCode, req.params.code.toUpperCase()));
        if (!room) {
