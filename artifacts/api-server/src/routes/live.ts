...
router.post("/live/start", requireAuth, async (req: any, res) => {
  try {
    ...
    const [stream] = await db
      ...
      .returning();
    const [host] = await readDb.select().from(usersTable).where(eq(usersTable.id, req.session.userId));
...
router.patch("/live/:id/end", requireAuth, async (req: any, res) => {
  try {
    ...
    const [stream] = await readDb.select().from(liveStreamsTable).where(eq(liveStreamsTable.id, id));
...
router.get("/live/streams", async (req, res) => {
  try {
    const streams = await readDb.select().from(liveStreamsTable).where(eq(liveStreamsTable.status, "active")).orderBy(liveStreamsTable.startedAt);
    const enriched = await Promise.all(streams.map(async (s) => {
      const [host] = await readDb.select().from(usersTable).where(eq(usersTable.id, s.hostId));
...
router.get("/live/:id", async (req, res) => {
  try {
    ...
    const [stream] = await readDb.select().from(liveStreamsTable).where(eq(liveStreamsTable.id, id));
    if (!stream) { res.status(404).json({ error: "Not found" }); return; }
    const [host] = await readDb.select().from(usersTable).where(eq(usersTable.id, stream.hostId));
...
router.patch("/live/:id/viewers", async (req, res) => {
  try {
    ...
    const [stream] = await readDb.select({ viewerCount: liveStreamsTable.viewerCount }).from(liveStreamsTable).where(eq(liveStreamsTable.id, id));
    if (!stream) { res.status(404).json({ error: "Not found" }); return; }
