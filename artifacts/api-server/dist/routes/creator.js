...
// POST /creator/plans
router.post("/creator/plans", requireAuth, async (req, res) => {
    try {
        const { name, description, price, perks } = req.body;
        if (!name?.trim() || !price || price < 1000) {
            res.status(400).json({ error: "Nomi va minimal narx (1000 tiyin) talab qilinadi" });
            return;
        }
...
router.post("/creator/subscribe/:planId", requireAuth, async (req, res) => {
    try {
        ...
        if (plan.creatorId === req.session.userId) {
        ...
        if (subscriberWallet.balance < plan.price) {
            res.status(400).json({ error: "Hamyonida mablag' yetarli emas", required: plan.price, balance: subscriberWallet.balance });
            return;
        }
...
router.delete("/creator/subscribe/:planId", requireAuth, async (req, res) => {
    try {
        const planId = Number(req.params.planId);
...
router.get("/creator/subscriptions", requireAuth, async (req, res) => {
    try {
...
router.get("/creator/check-subscription/:creatorId", requireAuth, async (req, res) => {
    try {
        const creatorId = Number(req.params.creatorId);
