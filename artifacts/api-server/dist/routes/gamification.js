...
async function checkAndGrantTitle(userId, totalEarned) {
    ...
    if (!newTitle)
        return;
    const existing = await db.select().from(userTitlesTable).where(and(eq(userTitlesTable.userId, userId), eq(userTitlesTable.title, newTitle)));
    if (existing.length === 0) {
        await db.insert(userTitlesTable).values({ userId, title: newTitle });
    }
}
router.get("/gamification/balance", requireAuth, async (req, res) => {
    try {
...
router.get("/gamification/quests", requireAuth, async (req, res) => {
    try {
...
router.post("/gamification/quests/:key/progress", requireAuth, async (req, res) => {
    try {
...
router.post("/gamification/quests/:key/claim", requireAuth, async (req, res) => {
    try {
        const { key } = req.params;
        const today = todayDate();
        const quest = await db.query.dailyQuestsTable.findFirst({ where: eq(dailyQuestsTable.key, key) });
...
router.post("/gamification/streak/touch", requireAuth, async (req, res) => {
    try {
        const today = todayDate();
        const streak = await getOrCreateStreak(req.session.userId);
