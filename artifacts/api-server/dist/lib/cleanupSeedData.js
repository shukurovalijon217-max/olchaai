...
export async function cleanupSeedData() {
    ...
    try {
        const { rows: existingUsers } = await client.query("SELECT id, username FROM users WHERE username = ANY($1::text[])", [SEED_USERNAMES]);
        const { rows: existingGroups } = await client.query("SELECT id, name FROM groups WHERE name = ANY($1::text[])", [SEED_GROUP_NAMES]);
        ...
        logger.info({ userCount: existingUsers.length, groupCount: existingGroups.length }, "Seed data cleanup: done");
    }
    catch (err) {
