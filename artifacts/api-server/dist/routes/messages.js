...
import { usersTable, chatConversationsTable, chatParticipantsTable, chatMessagesTable } from "@workspace/db";
...
async function notifyGo(userId, event, payload) {
    try {
...
function isFocusShieldMuted(fs, senderId) {
    if (!fs?.enabled)
        return false;
    if (fs.allowedUserIds?.includes(senderId))
        return false;
...
async function finalizeDueScheduledMessages(conversationId) {
    ...
    const [dueMsg] = await db.select().from(chatMessagesTable)
        .where(and(eq(chatMessagesTable.conversationId, conversationId), lte(chatMessagesTable.scheduledAt, now)))
        .orderBy(desc(chatMessagesTable.scheduledAt))
        .limit(1);
    if (!dueMsg || !dueMsg.scheduledAt)
        return;
    if (dueMsg.scheduledAt <= conv.updatedAt)
...
router.post("/conversations", async (req, res) => {
    ...
    try {
        ...
        const ids = Array.from(new Set([...(participantIds ?? []), userId]));
        if (!ids.includes(userId))
            ids.push(userId);
        const [conv] = await db.insert(chatConversationsTable).values({}).returning();
        await Promise.all(ids.map(pid => db.insert(chatParticipantsTable).values({ conversationId: conv.id, userId: pid })));
...
router.get("/conversations/:id/messages", async (req, res) => {
    ...
    try {
        ...
        const msgs = await db.select().from(chatMessagesTable)
            .where(and(eq(chatMessagesTable.conversationId, id), or(isNull(chatMessagesTable.scheduledAt), lte(chatMessagesTable.scheduledAt, now), eq(chatMessagesTable.senderId, userId))))
...
router.post("/conversations/:id/messages", async (req, res) => {
    ...
    try {
        ...
        /* Quest tracker */
        if (!isFutureScheduled)
            void trackQuestAction(senderId, "send_message");
    }
    catch (err) {
        req.log.error(err);
        res.status(500).json({ error: "Internal server error" });
...
router.delete("/conversations/:id/messages/:msgId", async (req, res) => {
    ...
    try {
        ...
        await db.delete(chatMessagesTable).where(and(eq(chatMessagesTable.id, msgId), eq(chatMessagesTable.conversationId, conversationId), eq(chatMessagesTable.senderId, userId)));
        res.status(204).end();
    }
    catch (err) {
        req.log.error(err);
