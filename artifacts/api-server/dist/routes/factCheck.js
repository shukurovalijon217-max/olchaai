...
router.get("/factcheck/:postId", async (req, res) => {
    try {
...
router.post("/factcheck/:postId", requireAuth, async (req, res) => {
    try {
        const postId = parseInt(req.params.postId);
...
Respond in JSON format:
{
  ...
  "sources": "comma-separated source hints or empty string"
        ...
        let parsed = { verdict: "unverifiable", confidence: 0, explanation: "Tekshirib bo'lmadi", sources: "" };
        try {
...
router.get("/credibility/leaderboard", async (req, res) => {
    try {
        const rows = await db
            .select({
...
router.get("/credibility/:userId", async (req, res) => {
    try {
